        ...
        <div tw="flex flex-col h-full w-full bg-white" style={{ fontFamily: `Inter, Helvetica, Arial, sans-serif` }}>
          ...
          <div tw="flex flex-col items-center justify-between h-[41.67%] bib__middle" style={{ backgroundColor: theme.background }}>
            <div tw="flex flex-row justify-center w-full text-3xl font-bold h-8 pt-8">
              ...
              <div tw="flex flex-col items-center px-8 bib__number">#WeRunTogether</div>
          ...
          <div tw="flex items-center justify-center px-16 py-12 h-1/4 w-full bg-black text-[8rem] text-white uppercase bib__lower">
            {searchParams.has("name") ? (<div className="bib__name">{name}</div>) : (<img src={`${process.env.NEXT_PUBLIC_URL}/img/nb.png`} alt="New Balance" tw="object-contain" />)}
          </div>
        </div>
      ),
      {
